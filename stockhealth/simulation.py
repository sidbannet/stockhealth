...
class MonteCarloWithTraining(MonteCarlo):
    ...
    def __init__(
        ...
        volatility = trained_model.history['volatility']
        beta = np.float(0)
        kappa = np.float(0)
        epsilon = np.float(0)
        super().__init__(
            model=Model(
                mew=mew,
                S0=price,
                volatility=volatility,  # //todo: This is based on average daily volatility
                beta=beta,
                kappa=kappa,
                epsilon=epsilon,
...
class MonteCarlosWithHeston(MonteCarlo):
    ...
    def __init__(
        ...
        historical_volatility = trained_model.history['volatility']
        price = trained_model.history['latest close']
